...
export type ReloadStrategyContext = {
  ...
  /** Resolved ladder tier for mode `user` (L0/L1). */
  ...
  /** May return a Promise — deep strategy awaits navigation. */
  hardReloadCacheBust?: () => void | Promise<void>;
};
...
export async function defaultUserReloadStrategy(ctx: ReloadStrategyContext): Promise<void> {
  // L0 / waiting SW must skipWaiting — never soft-reload deploy path when serverStartedAt lingers.
  if (ctx.reason === 'sw_waiting' || ctx.selectedTier === 0) {
...
export async function defaultDeepRecoveryStrategy(ctx: ReloadStrategyContext): Promise<void> {
  ...
  if (ctx.hardReloadCacheBust) {
    await Promise.resolve(ctx.hardReloadCacheBust());
    return;
  }
  if (ctx.serverStartedAt) {
    ctx.hardReload(ctx.serverStartedAt);
