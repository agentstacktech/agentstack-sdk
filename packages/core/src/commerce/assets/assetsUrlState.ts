...
export type AssetsWorkspaceMode = 'catalog' | 'import' | 'commerce' | 'studio';

export type CommerceHubTab = 'listings' | 'orders' | 'tools';

export type StudioTab = 'quick' | 'bulk' | 'ai' | 'fill' | 'hosted';
...
const STUDIO_TABS: StudioTab[] = ['quick', 'bulk', 'ai', 'fill', 'hosted'];
...
function parseStudioTab(raw: string | null): StudioTab {
  if (raw && STUDIO_TABS.includes(raw as StudioTab)) return raw as StudioTab;
  return 'quick';
}

function parseWorkspaceMode(raw: string | null): AssetsWorkspaceMode {
  if (raw === 'import') return 'import';
  if (raw === 'commerce') return 'commerce';
...
export function parseAssetsSearchParams(sp: URLSearchParams): AssetsUrlState {
  ...
  return {
    ...
    mode: parseWorkspaceMode(sp.get('mode')),
...
export function writeAssetsSearchParams(
  ...
  if (patch.studioTab !== undefined) {
    if (patch.mode === 'studio' || sp.get('mode') === 'studio') {
